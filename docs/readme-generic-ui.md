...
- `"listView"`: Defines how resources are displayed in table format
  ...
  - `"filters"`: Optional array of `FieldFilterDefinition` objects rendered as filter tabs above the table. Each entry defines a tab that scopes the list to rows whose `"property"` equals the given `"value"` (sent to the OpenSearch backend as `<property>=<value>`). An **"All" tab is prepended automatically by the underlying table card** — you do not need to author one. Properties per entry:
    ...
    - `"default"`: Optional boolean; when `true`, this tab is selected on initial render. If omitted on every entry, the auto-prepended "All" tab is selected.
