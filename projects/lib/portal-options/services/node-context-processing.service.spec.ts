...
describe('NodeContextProcessingServiceImpl', () => {
  ...
  describe('processNodeContext', () => {
    ...
    it('should handle missing organization in accountInfo', async () => {
      const accountInfoWithoutOrg: AccountInfo = {
        spec: {
          clusterInfo: {
            ca: 'certificate-data',
          },
      ...
      accountInfoService.read.mockReturnValue(of(accountInfoWithoutOrg));
      ...
      expect(mockContext.organizationId).toContain('undefined/');
    });

    it('should handle missing clusterInfo in accountInfo', async () => {
      const accountInfoWithoutCA: AccountInfo = {
        spec: {
          organization: {
            originClusterId: 'cluster-org-1',
          },
      ...
      accountInfoService.read.mockReturnValue(of(accountInfoWithoutCA));
      ...
      expect(mockContext.kcpCA).toBe(btoa('undefined'));
    });

    it('should handle missing account in accountInfo', async () => {
      const accountInfoWithoutAccount: AccountInfo = {
        spec: {
          organization: {
            originClusterId: 'cluster-org-1',
          },
      ...
      accountInfoService.read.mockReturnValue(of(accountInfoWithoutAccount));
      ...
      expect(mockContext.entityId).toContain('undefined/');
    });
